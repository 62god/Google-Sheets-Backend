function initPlatformerGame() {
  ...
  const items = [
    // Resized trophy to 64x64, adjusted Y position so it rests cleanly on the rock
    { x: 450, y: 156, width: 64, height: 64, type: 'trophy', collected: false }
  ];
  ...
  function render() {
    ...
    // Draw items (Trophy) - Updated to use item.width and item.height instead of TILE_SIZE
    items.forEach(item => {
      if (!item.collected) {
